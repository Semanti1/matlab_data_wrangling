...
FR_IDX = Data.FR_IDX;
% campos([-32.6 -9962 148])
% campos([0 0 0])
P_dense2 = P_dense;
P_dense2(:,1)=P_dense2(:,1)+70;
plot3(P_dense(:,1), P_dense(:,2), P_dense(:,3))
% campos
% campos([4600 -2380.1 9660.3])
% figure
% plot3(P_dense(:,1), P_dense(:,2), P_dense(:,3))
% drawnow
% proj=camproj;
% refreshdata
figure
plot3(P_dense2(:,1), P_dense2(:,2), P_dense2(:,3))
% refreshdata(P_dense)
idx_boundary = boundary(P_dense2(:,1), P_dense2(:,3), 1);
P_FR_boundary = [P_dense2(idx_boundary,1), P_dense2(idx_boundary,3)];
...
%     campos([1000 50 100])
    ...
    if ~exist('ceasar_mat_2views') 
        mkdir ceasar_mat_2views
    end
    save_file = "ceasar_mat_2views/view8_"+pos_i+fileName;
